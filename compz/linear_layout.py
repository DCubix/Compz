...
class LinearLayout(LayoutBase):
	...
	def apply(self, comp):
		...
		if self.orientation == ITEM_ORI_VERTICAL:
			comp.width = width - (self.padding * 2)
			comp.position = [self.padding, self.padding + self.__h]
			self.__h += comp.height + self.spacing
		else:
			comp.height = height - (self.padding * 2)
			comp.position = [self.padding + self.__h, self.padding]
